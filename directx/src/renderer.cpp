...
void Renderer::LoadPipeline()
{
	...
	//ThrowIfFailed(device->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_DIRECT, IID_PPV_ARGS(&command_allocators)));
}
...
void Renderer::LoadAssets()
{
	...
	ThrowIfFailed(device->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, command_allocators[0].Get(), pipeline_state.Get(), IID_PPV_ARGS(&command_list)));
	ThrowIfFailed(command_list->Close());

	std::wstring w_folder = module_path.substr(0, pos);
	std::string folder = std::string(w_folder.begin(), w_folder.end());
	std::string input_file = folder + "\\CornellBox-Original.obj";
	...
	bool ret = tinyobj::LoadObj(&attrib, &shapes, &materials, &warn, &err, input_file.c_str(), folder.c_str(), true);
...
void Renderer::PopulateCommandList()
{
	...
	CD3DX12_CPU_DESCRIPTOR_HANDLE rtv_handler(rtv_heap->GetCPUDescriptorHandleForHeapStart(), frame_index, rtv_descriptor_size);
	command_list->OMSetRenderTargets(1, &rtv_handler, FALSE, nullptr);
	const float clear_color[] = { 0.f, 0.f, 0.f, 1.f };
	command_list->ClearRenderTargetView(rtv_handler, clear_color, 0, nullptr);
