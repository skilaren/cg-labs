...
int Win32Window::Run(Renderer* pRenderer, HINSTANCE hInstance, int nCmdShow)
{
	...
	WNDCLASSEX window_ñlass = {};
	window_ñlass.cbSize = sizeof(WNDCLASSEX);
	window_ñlass.style = CS_HREDRAW | CS_VREDRAW;
	window_ñlass.lpfnWndProc = WindowProc;
	window_ñlass.hInstance = hInstance;
	window_ñlass.hCursor = LoadCursor(NULL, IDC_ARROW);
	window_ñlass.lpszClassName = L"DXSampleClass";
	RegisterClassEx(&window_ñlass);

	// Create the window and store a handle to it.
	RECT window_rect = { 0, 0, static_cast<LONG>(pRenderer->GetWidth()), static_cast<LONG>(pRenderer->GetHeight()) };
	AdjustWindowRect(&window_rect, WS_OVERLAPPEDWINDOW, FALSE);
	hwnd = CreateWindow(
		window_ñlass.lpszClassName,
		pRenderer->GetTitle(),
		WS_OVERLAPPEDWINDOW,
		CW_USEDEFAULT,
		CW_USEDEFAULT,
